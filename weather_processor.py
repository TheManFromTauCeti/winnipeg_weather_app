...
class WeatherProcessor():
    ...
    def __init__(self):
        ...
        try:
            ...
            self.logger.error("WeatherProcessor::__init__::%s", error_message)
        ...
        try:
            ...
            while not final_month:
                try:
                    ...
                    weather = self.scrapper.retrieve_monthly_data(year, month)
                    ...
                    self.logger.error("WeatherProcessor::update::while_loop::%s", error_message)
            ...
            for key, value in weather.items():
                try:
                    ...
                    self.logger.error("WeatherProcessor::update::for_loop::%s", error_message)
            ...
            self.logger.error("WeatherProcessor::update::%s", error_message)
        ...
        try:
            ...
            while not is_finished:
                try:
                    ...
                    self.logger.error("WeatherProcessor::retrieve_all::while_loop::%s", error_message)

        except Exception as error_message:
            self.logger.error("WeatherProcessor::retrieve_all::%s", error_message)
    ...
    def line_plot(self, month, year):
        ...
        try:
            ...
            self.logger.error("WeatherProcessor::line_plot::%s", error_message)
        ...
        try:
            ...
            for entry in entries:
                try:
                    ...
                    self.logger.error("WeatherProcessor::box_plot::for_loop::%s", error_message)

            self.plotter.box_plot(weather_data, start_year, end_year)

        except Exception as error_message:
            self.logger.error("WeatherProcessor::box_plot::%s", error_message)

if __name__ == '__main__':

    logger = logging.getLogger("main")
    logger.setLevel(logging.DEBUG)
    fh = logging.handlers.RotatingFileHandler(filename="weatherprocessorapp.log",
                                                maxBytes=10485760,
                                                backupCount=10)
    ...
    try:
        ...
        print("*************************************************")
        print("*                                               *")
        print("* Adam's Stupendous Weather Processor           *")
        print("*                                               *")
        print("* Version 1.0                                   *")
        print("* Developed by Adam Lukowski                    *")
        ...
        logger.error("weather_processor::main::%s", error_message)
